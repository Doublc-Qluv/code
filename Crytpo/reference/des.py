...
def encrypt(text, key,flag=1):
    return crypt(text,key,flag)

def decrypt(text, key,flag=0):
    return crypt(text,key,flag)


if __name__=='__main__':
    key = "theworld" #必须为8位
    text = "WhatIlikeisthefuture"
    cipher=""
    plain=""
    ...
    for i in range(int(length / 4)):
        ...
        cipher = "".join([cipher, encrypt(tempText, key)])
    ...
    for i in range(int(length / 8)):
        ...
        plain = "".join([plain, decrypt(tempText, key)])
