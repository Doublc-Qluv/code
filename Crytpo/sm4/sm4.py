...
class CryptSM4(object):
    ...
    def one_round(self, sk, in_put):
        ...
        for idx in range(32):
            inter_output = []
            buf = self._f(ulbuf[idx], ulbuf[idx + 1], ulbuf[idx + 2], ulbuf[idx + 3], sk[idx])
            ulbuf[idx + 4] = buf
            inter_output += put_uint32_be(ulbuf[idx + 1])
            inter_output += put_uint32_be(ulbuf[idx + 2])
            inter_output += put_uint32_be(ulbuf[idx + 3])
            inter_output += put_uint32_be(ulbuf[idx + 4])
            inter_result = list_to_bytes(inter_output)
            print "%02d %r" % (idx+1, inter_result)
...
list_to_bytes = lambda data: ''.join([chr(i) for i in data])

bytes_to_list = lambda data: [ord(i) for i in data]
